...
// ModifyTargetConfig invokes the aegis.ModifyTargetConfig API synchronously
// api document: https://help.aliyun.com/api/aegis/modifytargetconfig.html
func (client *Client) ModifyTargetConfig(request *ModifyTargetConfigRequest) (response *ModifyTargetConfigResponse, err error) {
	response = CreateModifyTargetConfigResponse()
	err = client.DoAction(request, response)
	return
}

// ModifyTargetConfigWithChan invokes the aegis.ModifyTargetConfig API asynchronously
// api document: https://help.aliyun.com/api/aegis/modifytargetconfig.html
...
func (client *Client) ModifyTargetConfigWithChan(request *ModifyTargetConfigRequest) (<-chan *ModifyTargetConfigResponse, <-chan error) {
	responseChan := make(chan *ModifyTargetConfigResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.ModifyTargetConfig(request)
		if err != nil {
			errChan <- err
		} else {
...
// ModifyTargetConfigWithCallback invokes the aegis.ModifyTargetConfig API asynchronously
// api document: https://help.aliyun.com/api/aegis/modifytargetconfig.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) ModifyTargetConfigWithCallback(request *ModifyTargetConfigRequest, callback func(response *ModifyTargetConfigResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *ModifyTargetConfigResponse
		var err error
		defer close(result)
		response, err = client.ModifyTargetConfig(request)
...
func CreateModifyTargetConfigRequest() (request *ModifyTargetConfigRequest) {
	request = &ModifyTargetConfigRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("aegis", "2016-11-11", "ModifyTargetConfig", "vipaegis", "openAPI")
	return
}
...
func CreateModifyTargetConfigResponse() (response *ModifyTargetConfigResponse) {
	response = &ModifyTargetConfigResponse{
