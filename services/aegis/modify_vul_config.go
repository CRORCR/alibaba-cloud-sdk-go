...
// ModifyVulConfig invokes the aegis.ModifyVulConfig API synchronously
// api document: https://help.aliyun.com/api/aegis/modifyvulconfig.html
func (client *Client) ModifyVulConfig(request *ModifyVulConfigRequest) (response *ModifyVulConfigResponse, err error) {
	response = CreateModifyVulConfigResponse()
	err = client.DoAction(request, response)
	return
}

// ModifyVulConfigWithChan invokes the aegis.ModifyVulConfig API asynchronously
// api document: https://help.aliyun.com/api/aegis/modifyvulconfig.html
...
func (client *Client) ModifyVulConfigWithChan(request *ModifyVulConfigRequest) (<-chan *ModifyVulConfigResponse, <-chan error) {
	responseChan := make(chan *ModifyVulConfigResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.ModifyVulConfig(request)
		if err != nil {
			errChan <- err
		} else {
...
// ModifyVulConfigWithCallback invokes the aegis.ModifyVulConfig API asynchronously
// api document: https://help.aliyun.com/api/aegis/modifyvulconfig.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) ModifyVulConfigWithCallback(request *ModifyVulConfigRequest, callback func(response *ModifyVulConfigResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *ModifyVulConfigResponse
		var err error
		defer close(result)
		response, err = client.ModifyVulConfig(request)
...
func CreateModifyVulConfigRequest() (request *ModifyVulConfigRequest) {
	request = &ModifyVulConfigRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("aegis", "2016-11-11", "ModifyVulConfig", "vipaegis", "openAPI")
	return
}
...
func CreateModifyVulConfigResponse() (response *ModifyVulConfigResponse) {
	response = &ModifyVulConfigResponse{
