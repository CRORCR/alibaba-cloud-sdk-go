...
// PreCreateMedia invokes the cloudcallcenter.PreCreateMedia API synchronously
// api document: https://help.aliyun.com/api/cloudcallcenter/precreatemedia.html
func (client *Client) PreCreateMedia(request *PreCreateMediaRequest) (response *PreCreateMediaResponse, err error) {
	response = CreatePreCreateMediaResponse()
	err = client.DoAction(request, response)
	return
}

// PreCreateMediaWithChan invokes the cloudcallcenter.PreCreateMedia API asynchronously
// api document: https://help.aliyun.com/api/cloudcallcenter/precreatemedia.html
...
func (client *Client) PreCreateMediaWithChan(request *PreCreateMediaRequest) (<-chan *PreCreateMediaResponse, <-chan error) {
	responseChan := make(chan *PreCreateMediaResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.PreCreateMedia(request)
		if err != nil {
			errChan <- err
		} else {
...
// PreCreateMediaWithCallback invokes the cloudcallcenter.PreCreateMedia API asynchronously
// api document: https://help.aliyun.com/api/cloudcallcenter/precreatemedia.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) PreCreateMediaWithCallback(request *PreCreateMediaRequest, callback func(response *PreCreateMediaResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *PreCreateMediaResponse
		var err error
		defer close(result)
		response, err = client.PreCreateMedia(request)
...
type PreCreateMediaResponse struct {
...
func CreatePreCreateMediaRequest() (request *PreCreateMediaRequest) {
	request = &PreCreateMediaRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("CloudCallCenter", "2017-07-05", "PreCreateMedia", "", "")
...
func CreatePreCreateMediaResponse() (response *PreCreateMediaResponse) {
	response = &PreCreateMediaResponse{
