...
// api document: https://help.aliyun.com/api/dataworks-public/listfileversions.html
func (client *Client) ListFileVersions(request *ListFileVersionsRequest) (response *ListFileVersionsResponse, err error) {
	response = CreateListFileVersionsResponse()
...
func (client *Client) ListFileVersionsWithChan(request *ListFileVersionsRequest) (<-chan *ListFileVersionsResponse, <-chan error) {
	responseChan := make(chan *ListFileVersionsResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.ListFileVersions(request)
		if err != nil {
			errChan <- err
		} else {
...
func (client *Client) ListFileVersionsWithCallback(request *ListFileVersionsRequest, callback func(response *ListFileVersionsResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *ListFileVersionsResponse
		var err error
		defer close(result)
		response, err = client.ListFileVersions(request)
...
type ListFileVersionsResponse struct {
	*responses.BaseResponse
	...
	Data           DataInListFileVersions `json:"Data" xml:"Data"`
}
...
func CreateListFileVersionsRequest() (request *ListFileVersionsRequest) {
	...
	request.InitWithApiInfo("dataworks-public", "2020-05-18", "ListFileVersions", "dide", "openAPI")
