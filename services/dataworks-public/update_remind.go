...
// api document: https://help.aliyun.com/api/dataworks-public/updateremind.html
func (client *Client) UpdateRemind(request *UpdateRemindRequest) (response *UpdateRemindResponse, err error) {
	response = CreateUpdateRemindResponse()
...
func (client *Client) UpdateRemindWithChan(request *UpdateRemindRequest) (<-chan *UpdateRemindResponse, <-chan error) {
	responseChan := make(chan *UpdateRemindResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.UpdateRemind(request)
		if err != nil {
			errChan <- err
		} else {
...
func (client *Client) UpdateRemindWithCallback(request *UpdateRemindRequest, callback func(response *UpdateRemindResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *UpdateRemindResponse
		var err error
		defer close(result)
		response, err = client.UpdateRemind(request)
...
func CreateUpdateRemindRequest() (request *UpdateRemindRequest) {
	request = &UpdateRemindRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("dataworks-public", "2020-05-18", "UpdateRemind", "dide", "openAPI")
