...
func (client *Client) CompileSortScriptWithChan(request *CompileSortScriptRequest) (<-chan *CompileSortScriptResponse, <-chan error) {
	responseChan := make(chan *CompileSortScriptResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.CompileSortScript(request)
		if err != nil {
			errChan <- err
		} else {
...
func (client *Client) CompileSortScriptWithCallback(request *CompileSortScriptRequest, callback func(response *CompileSortScriptResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *CompileSortScriptResponse
		var err error
		defer close(result)
		response, err = client.CompileSortScript(request)
...
func CreateCompileSortScriptRequest() (request *CompileSortScriptRequest) {
	request = &CompileSortScriptRequest{
		RoaRequest: &requests.RoaRequest{},
	}
	request.InitWithApiInfo("OpenSearch", "2017-12-25", "CompileSortScript", "/v4/openapi/app-groups/[appGroupIdentity]/apps/[appVersionId]/sort-scripts/[scriptName]/actions/compiling", "opensearch", "openAPI")
...
func CreateCompileSortScriptResponse() (response *CompileSortScriptResponse) {
	response = &CompileSortScriptResponse{
