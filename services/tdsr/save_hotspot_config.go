...
// SaveHotspotConfig invokes the tdsr.SaveHotspotConfig API synchronously
// api document: https://help.aliyun.com/api/tdsr/savehotspotconfig.html
func (client *Client) SaveHotspotConfig(request *SaveHotspotConfigRequest) (response *SaveHotspotConfigResponse, err error) {
	response = CreateSaveHotspotConfigResponse()
	err = client.DoAction(request, response)
	return
}

// SaveHotspotConfigWithChan invokes the tdsr.SaveHotspotConfig API asynchronously
// api document: https://help.aliyun.com/api/tdsr/savehotspotconfig.html
...
func (client *Client) SaveHotspotConfigWithChan(request *SaveHotspotConfigRequest) (<-chan *SaveHotspotConfigResponse, <-chan error) {
	responseChan := make(chan *SaveHotspotConfigResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.SaveHotspotConfig(request)
		if err != nil {
			errChan <- err
		} else {
...
// SaveHotspotConfigWithCallback invokes the tdsr.SaveHotspotConfig API asynchronously
// api document: https://help.aliyun.com/api/tdsr/savehotspotconfig.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) SaveHotspotConfigWithCallback(request *SaveHotspotConfigRequest, callback func(response *SaveHotspotConfigResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *SaveHotspotConfigResponse
		var err error
		defer close(result)
		response, err = client.SaveHotspotConfig(request)
...
func CreateSaveHotspotConfigRequest() (request *SaveHotspotConfigRequest) {
	request = &SaveHotspotConfigRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("tdsr", "2020-01-01", "SaveHotspotConfig", "", "")
...
func CreateSaveHotspotConfigResponse() (response *SaveHotspotConfigResponse) {
	response = &SaveHotspotConfigResponse{
