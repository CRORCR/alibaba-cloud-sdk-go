...
// ListAccountConfig invokes the cloudwf.ListAccountConfig API synchronously
// api document: https://help.aliyun.com/api/cloudwf/listaccountconfig.html
func (client *Client) ListAccountConfig(request *ListAccountConfigRequest) (response *ListAccountConfigResponse, err error) {
	response = CreateListAccountConfigResponse()
	err = client.DoAction(request, response)
	return
}

// ListAccountConfigWithChan invokes the cloudwf.ListAccountConfig API asynchronously
// api document: https://help.aliyun.com/api/cloudwf/listaccountconfig.html
...
func (client *Client) ListAccountConfigWithChan(request *ListAccountConfigRequest) (<-chan *ListAccountConfigResponse, <-chan error) {
	responseChan := make(chan *ListAccountConfigResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.ListAccountConfig(request)
		if err != nil {
			errChan <- err
		} else {
...
// ListAccountConfigWithCallback invokes the cloudwf.ListAccountConfig API asynchronously
// api document: https://help.aliyun.com/api/cloudwf/listaccountconfig.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) ListAccountConfigWithCallback(request *ListAccountConfigRequest, callback func(response *ListAccountConfigResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *ListAccountConfigResponse
		var err error
		defer close(result)
		response, err = client.ListAccountConfig(request)
...
func CreateListAccountConfigRequest() (request *ListAccountConfigRequest) {
	request = &ListAccountConfigRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("cloudwf", "2017-03-28", "ListAccountConfig", "cloudwf", "openAPI")
	return
}
