...
// ListApPosition invokes the cloudwf.ListApPosition API synchronously
// api document: https://help.aliyun.com/api/cloudwf/listapposition.html
func (client *Client) ListApPosition(request *ListApPositionRequest) (response *ListApPositionResponse, err error) {
	response = CreateListApPositionResponse()
	err = client.DoAction(request, response)
	return
}

// ListApPositionWithChan invokes the cloudwf.ListApPosition API asynchronously
// api document: https://help.aliyun.com/api/cloudwf/listapposition.html
...
func (client *Client) ListApPositionWithChan(request *ListApPositionRequest) (<-chan *ListApPositionResponse, <-chan error) {
	responseChan := make(chan *ListApPositionResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.ListApPosition(request)
		if err != nil {
			errChan <- err
		} else {
...
// ListApPositionWithCallback invokes the cloudwf.ListApPosition API asynchronously
// api document: https://help.aliyun.com/api/cloudwf/listapposition.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) ListApPositionWithCallback(request *ListApPositionRequest, callback func(response *ListApPositionResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *ListApPositionResponse
		var err error
		defer close(result)
		response, err = client.ListApPosition(request)
...
func CreateListApPositionRequest() (request *ListApPositionRequest) {
	request = &ListApPositionRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("cloudwf", "2017-03-28", "ListApPosition", "cloudwf", "openAPI")
	return
}
...
func CreateListApPositionResponse() (response *ListApPositionResponse) {
	response = &ListApPositionResponse{
