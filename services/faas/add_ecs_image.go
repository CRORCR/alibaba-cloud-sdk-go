package faas

//Licensed under the Apache License, Version 2.0 (the "License");
//you may not use this file except in compliance with the License.
//You may obtain a copy of the License at
//
//http://www.apache.org/licenses/LICENSE-2.0
//
//Unless required by applicable law or agreed to in writing, software
//distributed under the License is distributed on an "AS IS" BASIS,
//WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
//See the License for the specific language governing permissions and
//limitations under the License.
//
// Code generated by Alibaba Cloud SDK Code Generator.
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

import (
	"github.com/CRORCR/alibaba-cloud-sdk-go/sdk/requests"
	"github.com/CRORCR/alibaba-cloud-sdk-go/sdk/responses"
)

// AddEcsImage invokes the faas.AddEcsImage API synchronously
// api document: https://help.aliyun.com/api/faas/addecsimage.html
func (client *Client) AddEcsImage(request *AddEcsImageRequest) (response *AddEcsImageResponse, err error) {
	response = CreateAddEcsImageResponse()
	err = client.DoAction(request, response)
	return
}

// AddEcsImageWithChan invokes the faas.AddEcsImage API asynchronously
// api document: https://help.aliyun.com/api/faas/addecsimage.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) AddEcsImageWithChan(request *AddEcsImageRequest) (<-chan *AddEcsImageResponse, <-chan error) {
	responseChan := make(chan *AddEcsImageResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.AddEcsImage(request)
		if err != nil {
			errChan <- err
		} else {
			responseChan <- response
		}
	})
	if err != nil {
		errChan <- err
		close(responseChan)
		close(errChan)
	}
	return responseChan, errChan
}

// AddEcsImageWithCallback invokes the faas.AddEcsImage API asynchronously
// api document: https://help.aliyun.com/api/faas/addecsimage.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) AddEcsImageWithCallback(request *AddEcsImageRequest, callback func(response *AddEcsImageResponse, err error)) <-chan int {
	result := make(chan int, 1)
	err := client.AddAsyncTask(func() {
		var response *AddEcsImageResponse
		var err error
		defer close(result)
		response, err = client.AddEcsImage(request)
		callback(response, err)
		result <- 1
	})
	if err != nil {
		defer close(result)
		callback(nil, err)
		result <- 0
	}
	return result
}

// AddEcsImageRequest is the request struct for api AddEcsImage
type AddEcsImageRequest struct {
	*requests.RpcRequest
	ShellUniqueId string `position:"Query" name:"ShellUniqueId"`
	ToolsVersion  string `position:"Query" name:"ToolsVersion"`
	ECSImageName  string `position:"Query" name:"ECSImageName"`
}

// AddEcsImageResponse is the response struct for api AddEcsImage
type AddEcsImageResponse struct {
	*responses.BaseResponse
	RequestId     string `json:"RequestId" xml:"RequestId"`
	ImageId       string `json:"ImageId" xml:"ImageId"`
	ShellUniqueId string `json:"ShellUniqueId" xml:"ShellUniqueId"`
	ECSImageName  string `json:"ECSImageName" xml:"ECSImageName"`
	ToolsVersion  string `json:"ToolsVersion" xml:"ToolsVersion"`
}

// CreateAddEcsImageRequest creates a request to invoke AddEcsImage API
func CreateAddEcsImageRequest() (request *AddEcsImageRequest) {
	request = &AddEcsImageRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("faas", "2020-02-17", "AddEcsImage", "faas", "openAPI")
	request.Method = requests.POST
	return
}

// CreateAddEcsImageResponse creates a response to parse from AddEcsImage response
func CreateAddEcsImageResponse() (response *AddEcsImageResponse) {
	response = &AddEcsImageResponse{
		BaseResponse: &responses.BaseResponse{},
	}
	return
}
