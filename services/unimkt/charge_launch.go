...
func (client *Client) ChargeLaunchWithChan(request *ChargeLaunchRequest) (<-chan *ChargeLaunchResponse, <-chan error) {
	...
	err := client.AddAsyncTask(func() {
		...
		defer close(errChan)
		response, err := client.ChargeLaunch(request)
		if err != nil {
			errChan <- err
		} else {
...
func (client *Client) ChargeLaunchWithCallback(request *ChargeLaunchRequest, callback func(response *ChargeLaunchResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *ChargeLaunchResponse
		var err error
		defer close(result)
		response, err = client.ChargeLaunch(request)
...
type ChargeLaunchRequest struct {
	*requests.RpcRequest
...
type ChargeLaunchResponse struct {
...
func CreateChargeLaunchRequest() (request *ChargeLaunchRequest) {
	request = &ChargeLaunchRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("UniMkt", "2018-12-12", "ChargeLaunch", "uniMkt", "openAPI")
