...
// AddRoleCellToRole invokes the oam.AddRoleCellToRole API synchronously
// api document: https://help.aliyun.com/api/oam/addrolecelltorole.html
func (client *Client) AddRoleCellToRole(request *AddRoleCellToRoleRequest) (response *AddRoleCellToRoleResponse, err error) {
	response = CreateAddRoleCellToRoleResponse()
	err = client.DoAction(request, response)
	return
}

// AddRoleCellToRoleWithChan invokes the oam.AddRoleCellToRole API asynchronously
// api document: https://help.aliyun.com/api/oam/addrolecelltorole.html
...
func (client *Client) AddRoleCellToRoleWithChan(request *AddRoleCellToRoleRequest) (<-chan *AddRoleCellToRoleResponse, <-chan error) {
	responseChan := make(chan *AddRoleCellToRoleResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.AddRoleCellToRole(request)
		if err != nil {
			errChan <- err
		} else {
...
// AddRoleCellToRoleWithCallback invokes the oam.AddRoleCellToRole API asynchronously
// api document: https://help.aliyun.com/api/oam/addrolecelltorole.html
// asynchronous document: https://help.aliyun.com/document_detail/66220.html
func (client *Client) AddRoleCellToRoleWithCallback(request *AddRoleCellToRoleRequest, callback func(response *AddRoleCellToRoleResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *AddRoleCellToRoleResponse
		var err error
		defer close(result)
		response, err = client.AddRoleCellToRole(request)
...
func CreateAddRoleCellToRoleRequest() (request *AddRoleCellToRoleRequest) {
	request = &AddRoleCellToRoleRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("Oam", "2017-01-01", "AddRoleCellToRole", "", "")
...
func CreateAddRoleCellToRoleResponse() (response *AddRoleCellToRoleResponse) {
	response = &AddRoleCellToRoleResponse{
